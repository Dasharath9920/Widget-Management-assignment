...
const AddWidgetModal = ({ isOpen, closeModal, onAddWidget}: { isOpen: boolean, closeModal: () => void, onAddWidget: () => void}) => {
      ...
      <div className="flex min-h-full items-center justify-center p-4 sm:p-0">
        <div className="relative transform overflow-hidden rounded-lg bg-white text-left transition-all">
          ...
          <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
            <button type="button" className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 ring-1 shadow-xs ring-gray-500 ring-inset hover:bg-gray-50 sm:mt-0 sm:w-auto" onClick={() => closeModal()}>Cancel</button>
          </div>
        </div>
      </div>
    </div>
